...
class AnalyticThread(threading.Thread):
    def __init__(self, name, pad_names, workQueue, queueLock):
...
def receiving_data():
    ...
    if flask_request.method == 'POST':
        pad_names = flask_request.get_json()['pad_names']
        ...
        workQueue = Queue(1)
        queueLock = threading.Lock()
        myThread = AnalyticThread("Analytics_thread", pad_names, workQueue, queueLock)
