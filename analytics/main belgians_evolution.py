...
for pad_name in pads:
    ...
    thresholds = np.linspace(start_time, end_time, 10)
    for num_th, threshold in enumerate(thresholds[:9]):
        new_pad, new_elem_ops = pad.pad_at_timestamp(threshold)
        print(len(new_pad.operations))
        if not new_pad.operations:
            continue
        ...
        print("PAD:", pad_name)
        print("TEXT:")
        print(new_pad.get_text())

        print('\nCOLORED TEXT BY AUTHOR')
        print(new_pad.display_text_colored_by_authors())

        print('\nCOLORED TEXT BY OPS')
        print(new_pad.display_text_colored_by_ops())

        print('\nSCORES')
        print('User proportion per paragraph score', new_pad.user_participation_paragraph_score())
        print('Proportion score:', new_pad.prop_score())
        print('Synchronous score:', new_pad.sync_score()[0])
        print('Alternating score:', new_pad.alternating_score())
        print('Break score day:', new_pad.break_score('day'))
        print('Break score short:', new_pad.break_score('short'))
        print('Overall write type score:', new_pad.type_overall_score('write'))
        print('Overall paste type score:', new_pad.type_overall_score('paste'))
        print('Overall delete type score:', new_pad.type_overall_score('delete'))
        print('Overall edit type score:', new_pad.type_overall_score('edit'))
        print('User write score:', new_pad.user_type_score('write'))
        print('User paste score:', new_pad.user_type_score('paste'))
        print('User delete score:', new_pad.user_type_score('delete'))
        print('User edit score:', new_pad.user_type_score('edit'))
