...
def build_operations_from_elem_ops(list_of_elem_ops_per_pad,
    ...
    for pad_name in list_of_elem_ops_per_pad:
        ...
        pad = pads.get(pad_name, Pad(pad_name))

        # If we have operations in standby, we get them.
        dic_author_current_operations = dic_author_current_operations_per_pad.get(
            pad_name,
            dict()
        )

        elem_ops_treated = elem_ops_treated_per_pad.get(pad_name, [])
        ...
        for elem_op in list_of_elem_ops_per_pad[pad_name]:
            if elem_op.operation_type == "add" and "\n" in elem_op.text_to_add:
                ...
                if first_elem_op_txt != "":
                    ...
                    elem_ops_treated.append(first_elem_op)
                ...
                for idx, txt in enumerate(elem_op_txts):
                    if txt is not "":
                        ...
                        elem_ops_treated.append(new_elem)
                ...
                if last_elem_op_txt != "":
                    ...
                    elem_ops_treated.append(last_elem_op)
                ...
                elem_ops_treated.append(elem_op)
                dic_author_current_operations = treat_op(elem_op,
                                                         dic_author_current_operations,
                                                         pad,
                                                         maximum_time_between_elem_ops)
        ...
        elem_ops_treated_per_pad[pad_name] = elem_ops_treated
